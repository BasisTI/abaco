...
public class BaselineSchedule {
	...
	public void verificaPorHora() { 
	    ...
	    if(configuracoes != null && !configuracoes.isEmpty()) {
	        ...
	        for (ConfiguracaoJobBaseline configuracaoJobBaseline : configuracoes) {
	            if(configuracaoJobBaseline.getSistema() == null) {
	                for (Sistema sistema : sistemas) {
	                    BaseLineSintetico baseLineSintetico = baselineSinteticoService.getBaseLineSintetico(sistema.getId(), configuracaoJobBaseline.getTipoEquipe().getId());
	                    if(baseLineSintetico != null) {
	                        baseLineSintetico = baselineSinteticoService.getBaseLineAnaliticoFDFT(sistema.getId(), configuracaoJobBaseline.getTipoEquipe().getId(), baseLineSintetico);
	                    }
	                }
	            }else if(configuracaoJobBaseline.getTipoEquipe() == null){
                    for (TipoEquipe tipoEquipe : tipoEquipes) {
                        BaseLineSintetico baseLineSintetico = baselineSinteticoService.getBaseLineSintetico(configuracaoJobBaseline.getSistema().getId(), tipoEquipe.getId());
                        if(baseLineSintetico != null) {
                            baseLineSintetico = baselineSinteticoService.getBaseLineAnaliticoFDFT(configuracaoJobBaseline.getSistema().getId(), tipoEquipe.getId(), baseLineSintetico);
                        }
                    }
	            }else {
	                BaseLineSintetico baseLineSintetico = baselineSinteticoService.getBaseLineSintetico(configuracaoJobBaseline.getSistema().getId(), configuracaoJobBaseline.getTipoEquipe().getId());
                    if(baseLineSintetico != null) {
                        baseLineSintetico = baselineSinteticoService.getBaseLineAnaliticoFDFT(configuracaoJobBaseline.getSistema().getId(), configuracaoJobBaseline.getTipoEquipe().getId(), baseLineSintetico);
                    }
	            }
	        }
	    }else {
	        ...
	        for (Sistema sistema : sistemas) {
	            for (TipoEquipe tipoEquipe : tipoEquipes) {
	                BaseLineSintetico baseLineSintetico = baselineSinteticoService.getBaseLineSintetico(sistema.getId(), tipoEquipe.getId());
	                if(baseLineSintetico != null) {
	                    baseLineSintetico = baselineSinteticoService.getBaseLineAnaliticoFDFT(sistema.getId(), tipoEquipe.getId(), baseLineSintetico);
	                }
	            }
	        }
	    }
