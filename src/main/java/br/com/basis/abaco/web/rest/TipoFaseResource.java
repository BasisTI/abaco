...
public class TipoFaseResource {
     ...
     *
     * @param tipoFase the tipoFase to create
     * @return the ResponseEntity with status 201 (Created) and with body the new tipoFase, or with status 400 (Bad Request) if the tipoFase has already an ID
    ...
    public ResponseEntity<TipoFase> createTipoFase(@RequestBody TipoFase tipoFase) throws URISyntaxException {
        ...
        if (tipoFase.getId() != null) {
            throw new BadRequestAlertException("A new tipoFase cannot already have an ID", ENTITY_NAME, "idexists");
        }
     ...
     *
     * @param tipoFase the tipoFase to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated tipoFase,
     * or with status 400 (Bad Request) if the tipoFase is not valid,
     * or with status 500 (Internal Server Error) if the tipoFase couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    public ResponseEntity<TipoFase> updateTipoFase(@RequestBody TipoFase tipoFase) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, tipoFase.getId().toString()))
            .body(result);
    }
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of tipoFases in body
     */
    @GetMapping("/tipo-fases")
    @Timed
    public ResponseEntity<List<TipoFase>> getAllTipoFases(Pageable pageable) {
        log.debug("REST request to get a page of TipoFases");
        Page<TipoFase> page = tipoFaseRepository.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/tipo-fases");
     ...
     *
     * @param id the id of the tipoFase to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the tipoFase, or with status 404 (Not Found)
     */
     ...
     *
     * @param id the id of the tipoFase to delete
    ...
    /**
     * SEARCH  /_search/tipo-fases?query=:query : search for the tipoFase corresponding
     * to the query.
     *
     * @param query the query of the tipoFase search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/tipo-fases")
    @Timed
    public ResponseEntity<List<TipoFase>> searchTipoFases(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of TipoFases for query {}", query);
        Page<TipoFase> page = tipoFaseSearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/tipo-fases");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
